...
- **创作基调**：开始创作前可选择基调（自由发挥 / 悬疑 / 温情 / 喜剧 / 暗黑），基调随分支与续写请求上送并注入 AI 提示词，方向与续写贴合基调的氛围与节奏；旧作品无基调字段时自动按「自由发挥」处理；
- **命运硬币**：分支卡片区「🎲 让命运决定」按钮随机选中一个方向，卡片 0.6s 高亮脉冲后自动进入续写，与手动选择行为完全一致；
- **故事统计**：面包屑旁实时显示当前激活路径的「N 字 · M 个分叉 · 第 K 段」（M 为全树中发生过分叉的节点数），随续写、回退、切换分支即时更新；
...
lib/
  ...
  errors.ts                # 统一错误响应
  rateLimit.ts             # 内存滑动窗口限流器（按 IP）
  accessCode.ts            # 可选访问口令校验
  tone.ts                  # 创作基调白名单 / 归一化

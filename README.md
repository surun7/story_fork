...
本项目为 AI 辅助编程产物：AI 完成绝大部分编码（三阶段迭代：M1 主路径 → M2 分支树与回退 → M3 持久化与导出），人工负责需求收敛、prompt 设计、验收与 bug 复核。

一个典型案例：**M2 阶段发现并修复了 M1 遗留的重试 bug**——M1 中续写失败后点击「重试」实际被 `phase !== "ready"` 的守卫拦下而无效（错误面板一直停留在 error 态）；M2 重构时将防连点职责移交 UI 禁用态，重试路径恢复可用。该问题正是通过人工逐条核对错误路径时发现的，说明「AI 写码 + 人工验收」的协作方式对交付质量至关重要。
